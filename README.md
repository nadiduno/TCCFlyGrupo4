...
**Demais explicações:**
1. Como definimos nosso púbico alvo? Para a análise de evasão escolar, é importante diferenciar ‘não ser aprovada’ de ‘evadir’ do curso. A evasão significa que a estudante tenha sido aprovada e, portanto, tenha efetivamente ingressado no percurso formativo. Dessa forma, uma estudante que não foi aprovada no processo de seleção ou ingresso não pode ser considerada evadida, pois não chegou a iniciar sua participação no curso.
Por esse motivo, o universo considerado nas estatísticas de evasão deste projeto é composto exclusivamente pelas estudantes aprovadas, para as quais é possível acompanhar sua trajetória ao longo da formação e identificar se houve conclusão ou abandono. As candidatas não aprovadas são desconsideradas dos cálculos de evasão, uma vez que sua situação representa uma etapa anterior ao início do curso.
Essa distinção é fundamental para evitar que diferentes situações sejam agrupadas em uma mesma métrica. Assim, a aprovação define quem pertence à nossa análise.
2. E o vazio estrutural dos dados? Durante a análise da base, foi identificado que aproximadamente 68,66% das células referentes às variáveis analisadas estão vazias. À primeira vista, esse percentual poderia indicar uma baixa adesão ao preenchimento dos questionários. No entanto, é necessário considerar que grande parte dessas ausências não representa necessariamente uma falta de resposta por parte das participantes.
O principal motivo para esse elevado número de células vazias é um vazio estrutural na coleta dos dados. Ao longo das diferentes turmas, foram utilizados formulários distintos, com perguntas e estruturas que sofreram alterações. Dessa forma, determinadas perguntas presentes em um formulário não estavam disponíveis em outros. Quando uma pergunta não fazia parte do formulário aplicado a determinada turma, não havia possibilidade de a participante respondê-la, resultando em uma célula vazia na base consolidada.
Essa distinção é importante porque uma célula vazia pode representar situações diferentes: uma pergunta que não foi respondida por uma participante ou uma pergunta que sequer fazia parte do formulário daquela turma. Portanto, não é adequado interpretar o percentual de 68,66% de valores ausentes exclusivamente como uma taxa de não resposta.
Para evidenciar esse comportamento, será apresentado no trabalho um heatmap de preenchimento por turma, permitindo visualizar quais variáveis estavam disponíveis e foram preenchidas em cada grupo. A representação ajuda a demonstrar visualmente que a distribuição dos dados ausentes está relacionada, em grande medida, às diferenças existentes entre os formulários utilizados ao longo das turmas.
3. Quanto aos dados parciais a respeito da evasão escolar? Os dados apresentados nesta etapa do projeto ainda são parciais, pois o status acadêmico — isto é, a identificação entre estudantes formadas e evadidas — está disponível, até o momento, somente para a aba T10_11_14_15_17. Nessa turma, foram identificadas 100 estudantes formadas e 66 evadidas, totalizando 166 estudantes com status definido e uma taxa de evasão parcial de aproximadamente 40%.
É importante destacar que esse percentual não representa ainda a taxa definitiva de evasão do projeto. O cálculo atual considera apenas a quantidade de estudantes cujo status já foi disponibilizado e validado. As demais sete turmas ainda não possuem seus respectivos status consolidados, o que impede, neste momento, a obtenção de um número total representativo de todas as participantes analisadas.
Essa limitação ocorre porque os dados necessários para determinar a situação final das estudantes ainda não foram disponibilizados ou consolidados para todas as turmas. Dessa forma, o número de formadas e evadidas poderá sofrer alterações à medida que essas informações forem incorporadas à base. A taxa de evasão será recalculada quando os status das demais turmas estiverem disponíveis, permitindo uma análise mais completa e representativa do cenário de evasão escolar identificado pelo projeto.
